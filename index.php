<?php
    ...
    try {
        // 1.ログイン状態をチェック、ログインユーザーの情報をセッションから取得
        session_start();
...
<html lang="ja">
    ...
    <body class="text-center bg-light">
        ...
        <form class="border rounded bg-white form-time-table" action="index.php">
            ...
            <select id="exampleFormControlSelect1" class="form-control rounded-pill mb-3" name="m" onchange="submit(this.form)">
                <option value="<?= date('Y-m') ?>"><?= date('Y/m') ?></option>
                <?php for($i = 1; $i < 12; $i++): ?>
                    <?php $target_yyyymm = strtotime("-{$i}months"); ?>
                    <?php echo $target_yyyymm; ?>
                ...
                <tbody>
                    <?php for ($i = 1;$i <= $day_count; $i++): ?>
                        ...
                        <tr>
                            ...
                            <td><?= $break_time ?></td>
                            <td><?= h($comment) ?></td>
                            <td><button type="button" class="btn btn-default h-auto py-0" data-toggle="modal" data-target="#inputModal" data-day="<?= $yyyymm.'-'. str_pad($i, 2, 0, STR_PAD_LEFT) ?>"><img  class='edit-button' src="img/edit-button.png" alt="編集"></button></td>
                        </tr>
                    <?php endfor; ?>
                </tbody>
            </table>
                        ...
                        <div class="modal-body">
                            <div class="container">
                                ...
                                <div class="form-group pt-3">
                                    <textarea class="form-control <?php if(isset($err['modal_comment'])) echo 'is-invalid';?>" id="modal_comment" row="5" placeholder="業務内容" name="modal_comment"><?= $modal_comment ?></textarea>
                                </div>
        ...
        <script>
            ...
            $('#inputModal').on('show.bs.modal', function(event) {
                ...
                /* エラー表示をクリア */
                $('#modal_start_time').remove('is-invalid')
                $('#modal_end_time').remove('is-invalid')
                $('#modal_break_time').remove('is-invalid')
                $('#modal_comment').remove('is-invalid')
            })
        </script>
    </body>
</html>